/*
** Player_2.hpp for cpp_bomberman in /home/camila/rendu/cpp_bomberman
**
** Made by 
** Login  <@epitech.eu>
**
** Started on  dim. juin 14 20:57:04 2015 
** Last update dim. juin 14 21:02:24 2015 
*/
...
private:
  ...
  bool _test = false;
public:
  Player2(std::vector<AObject *> *objects, int x, int z)
  {
    _objects = objects;
    _position.x = x;
    _position.y = -0.25;
  ...
  virtual bool initialize()
  {
    ...
    _model.setCurrentAnim("LibBomberman_linux_x64/assets/marvin.fbx", true);
    return (true);
  }
  virtual void update(gdl::Clock const &clockl, gdl::Input &input)
  {
    ...
    if (input.getKey(SDLK_s) && test_collid(glm::vec3(0, 0, -1) * static_cast<float>(clockl.getElapsed()) * _speed, 0) == false)
      {
    ...
    if (input.getKey(SDLK_z) && test_collid(glm::vec3(0, 0, 1) * static_cast<float>(clockl.getElapsed()) * _speed, 1) == false)
      {
	translate(glm::vec3(0, 0, 1) * static_cast<float>(clockl.getElapsed()) * _speed);
	test_collide_bonus();
    ...
    if (input.getKey(SDLK_d) && test_collid(glm::vec3(-1, 0, 0) * static_cast<float>(clockl.getElapsed()) * _speed, 2) == false)
      {
    ...
    if (input.getKey(SDLK_q) && test_collid(glm::vec3(1, 0, 0) * static_cast<float>(clockl.getElapsed()) * _speed, 3) == false)
      {
	translate(glm::vec3(1, 0, 0) * static_cast<float>(clockl.getElapsed()) * _speed);
	test_collide_bonus();
	_rotation = glm::vec3(0, 90, 0);
